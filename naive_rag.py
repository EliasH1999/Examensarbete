...
def load_pdf(pdf_path=None):
    if pdf_path is None:
        pdf_path = "random_manuals/S111_Modbus-register-table_EN_V2025-2.pdf ...pdf"
...
def split_docs(docs):
    ...
    return chunks
# Builds an in-memory Chroma vector store from the document chunks using OpenAIEmbeddings
def build_vectorstore(chunks):
    ...
    return vectorstore
# Retrieves relevant documents from the vector store based on the user query, combining results from both register-related and communication-related terms
def retrieve_combined_docs(vectorstore, query, k_each=8):
    retriever = vectorstore.as_retriever(search_kwargs={"k": k_each})

    register_docs = retriever.invoke(query)

    comm_query = " ".join(COMM_TERMS)
    comm_docs = retriever.invoke(comm_query)

    combined_docs = register_docs + comm_docs
    return combined_docs



# Formats the retrieved documents into a single string context for the LLM, including metadata and content of each chunk
def format_context(docs):
    parts = []
    for i, doc in enumerate(docs, start=1):
        parts.append(f"CHUNK {i}\n" f"Metadata: {doc.metadata}\n" f"Content:\n{doc.page_content}\n")
    return "\n".join(parts)
# Constructs a prompt for the LLM based on the provided context and user question, then invokes the model to get a response
...
# Main function to execute the RAG process: load PDF, split into chunks, build vector store, retrieve relevant docs based on user question, format context, and ask LLM for response
...
def save_retrieval_log(pdf_path, retrieved_docs):
    """Save which chunks were sent to the LLM to chunk_db/<manual>_retrieval.json."""
    db_dir = Path(__file__).resolve().parent / "chunk_db"
    db_dir.mkdir(exist_ok=True)
    stem = Path(pdf_path).stem if pdf_path else "unknown"
    chunk_entries = []
    ...
    log = [{
        "pass_name": "all_parameters",
        "parameters": ["all 15 parameters (single pass)"],
        "num_chunks_sent": len(chunk_entries),
...
def main():
    ...
    # Non-interactive mode: run once with CLI args and exit
    if _cli_args.manual and _cli_args.question:
        question = _cli_args.question
        output_path = _cli_args.output

        retrieved_docs = retrieve_combined_docs(vectorstore, question)

        # Save retrieval log
        save_retrieval_log(_cli_args.manual, retrieved_docs)

        context = format_context(retrieved_docs)
        response = ask_llm(question, context)

        lines = [ln.strip() for ln in response.splitlines() if ln.strip().startswith("subnetwork")]
